...
| API / agents host | **Bun + Hono** (existing starter `apps/api`): uploads, ADK agents, geometry, persistence | `bun:sqlite` needs the Bun runtime, so everything stateful lives here |
...
| Persistence | **SQLite via Drizzle + `bun:sqlite`** (uploads on disk) | Repo convention; ephemeral on Cloud Run is acceptable for the demo |
...
Auth, multi-floor, embosser/swell outputs, print-service integration, non-English labels, hand-drawn plan support, any database beyond SQLite, any infra beyond Cloud Run.
...
3. **Demo & production readiness (30%)** — scripted video, reproducible repo, live Cloud Run URL.
